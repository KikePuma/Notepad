...
void Language(char lang) {
	switch(lang) {
		case '0':
			...
			game.discard = "Hay una partida sin guardar, Â¿descartarla? (s/n) ";
			game.exit = "Hay una partida sin guardar, Â¿está seguro de que desea salir? (s/n) ";
		...
		case '1':
			...
			game.exit = "Hai unha partida sen gardar, está seguro de que desexa saÃƒÂ­r? (s/n) ";
