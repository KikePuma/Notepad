...
void insert() {
	...
	while(1) {
		...
		if(strlen(trying) != 4) { printf("La longitud de la apuesta es incorrecta."); continue; }
		
		for(i=0;i<4;i++) { if(trying[i] < '1' || trying[i] > '9') {	printf("La apuesta s\242lo debe contener d\241gitos."); break;	} }
		if (i != 4) continue;
		for(i=0;i<4;i++) { for(j=0;j<i;j++) if(trying[i] == trying[j]) {
			printf("Los d\241gitos de la apuesta deben ser diferentes."); i = 10; break;	} }
		if(i != 4) continue;
		break;
	}
...
int check() {
	...
	printf("Su apuesta es: %s <", thread.inserted); TXT(2," <");
	thread.level--;
	for(i = 0; i < 4; i++) {
		for(j = 0; j < 4; j++) {
